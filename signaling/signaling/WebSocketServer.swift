...
final class WebSocketServer {
    ...
    private let port: NWEndpoint.Port = 6049
    ...
    private func newConnectionHandler(_ connection: NWConnection) {
        ...
        self.broadcastClientsCount()
        print(connectedClients.first!.id)
    }
    ...
    private func broadcastClientsCount() {
        let countDict = [
            "type": "String",
            "payload": String(self.connectedClients.count)]
        let encoder = JSONEncoder()
        guard let countData = try? encoder.encode(countDict) else {
            print("Error encoding connection count")
            return
        }
        self.broadcast(data: countData, to: self.connectedClients)
        print((String(data: countData, encoding: .utf8) ?? "No countData found...") + "\n")
    }
}
